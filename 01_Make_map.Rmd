...
### Add the 4 sets of extra data and add 'Stasjon' variable  
...
# STATION_NAME, Stasjon (= type stasjon), STATION_CODE, LONGITUDE, LATITUDE
...
oa_stations1 <- readxl::read_excel(
  ...
  rename(
    Stasjon_orig = Stasjon,
    LATITUDE = `Latitude (decimal)`,
    LONGITUDE = `Longitude (decimal)`)

oa_stations2 <- oa_stations1 %>%
  filter(!is.na(LATITUDE)) %>%
  mutate(
    Stasjon = case_when(
      grepl("Kiel", Stasjon_orig) ~ "OA Ferrybox",
      grepl("Tromsø", Stasjon_orig) ~ "OA Ferrybox",
      grepl("VT", Stasjon_orig) ~ "OA Økokyst",
      grepl("VR", Stasjon_orig) ~ "OA Økokyst",
      `Instipro activity` %in% "KYST" ~ "OA Kyst",
      TRUE ~ "OA Kyst")
  )

# oa_stations2 %>% select(Type:`Instipro activity`, Stasjon) %>% View()
...
ggplot(oa_stations2, aes(LONGITUDE, LATITUDE, color = Stasjon)) +
  geom_point()
